...
describe("Mento", () => {
  ...
  const nOfFakeExchanges = Object.values(fakeExchangesByProviders).reduce((acc, curr) => acc + curr.length, 0);
  ...
  // @ts-ignore
  IExchangeProvider__factory.connect = jest.fn((exchangeProvider: string, _) => {
    return {
      getExchanges: () => fakeExchangesByProviders[exchangeProvider as keyof typeof fakeExchangesByProviders],
    };
  });
  ...
  const mockContractModule = jest.fn((contractAddr: string) => {
    ...
    const isErc20Contract = Object.keys(fakeSymbolsByTokenAddr).includes(contractAddr);

    if (isRegistryContract) {
      return fakeRegistryContract;
    } else if (isErc20Contract) {
      return {
        symbol: jest.fn(() => fakeSymbolsByTokenAddr[contractAddr as keyof typeof fakeSymbolsByTokenAddr]),
        populateTransaction: {
  ...
  describe("create", () => {
    ...
    it("should throw if the signer has no provider", async () => {
      await expect(Mento.create(signerWithoutProvider)).rejects.toThrow("Signer must be connected to a provider");
    });

    it("should throw if no signer or provider is provided", async () => {
      // @ts-ignore
      await expect(Mento.create()).rejects.toThrow("A valid signer or provider must be provided");
    });
  });
  ...
  describe("createWithBrokerAddress", () => {
    ...
    it("should throw if the signer has no provider", () => {
      expect(() => Mento.createWithBrokerAddress(fakeBrokerAddr, signerWithoutProvider)).toThrow(
  ...
  describe("getTradeablePairs", () => {
    it("should return an array of tuples with all the tradeable pairs", async () => {
      ...
      for (const [, mockedExchanges] of Object.entries(fakeExchangesByProviders)) {
        for (const mockedExchange of mockedExchanges) {
          ...
          const pairOfMockedExchange = [
            {
              address: asset0Addr,
              symbol: fakeSymbolsByTokenAddr[asset0Addr as keyof typeof fakeSymbolsByTokenAddr],
            },
            {
              address: asset1Addr,
              symbol: fakeSymbolsByTokenAddr[asset1Addr as keyof typeof fakeSymbolsByTokenAddr],
            },
          ];
  ...
  describe("getAmountIn", () => {
    it("should call broker.getAmountIn with the right parameters", async () => {
      ...
      for (const [mockedProvider, mockedExchanges] of Object.entries(fakeExchangesByProviders)) {
    ...
    it("should throw if no exchange is found for the given tokens", async () => {
      const testee = await Mento.create(provider);
      await expect(
        testee.getAmountIn(fakeCeloEURExchange.assets[0], "nonExistentAssetAddr", oneInWei),
      ).rejects.toThrow();
    });
  });
  ...
  describe("getAmountOut", () => {
    ...
    it("should throw if no exchange is found for the given tokens", async () => {
      const testee = await Mento.create(provider);
      await expect(
        testee.getAmountOut(fakeCeloUSDExchange.assets[1], "nonExistentAssetAddr", oneInWei),
      ).rejects.toThrow();
    });
  });
  ...
  describe("increaseTradingAllowance", () => {
    ...
    it("should throw if a signer is not provided", async () => {
      const testee = await Mento.create(provider);
      await expect(testee.increaseTradingAllowance(fakeCeloBRLExchange.assets[1], oneInWei)).rejects.toThrow(
  ...
  describe("swapIn", () => {
    it("should return a populated swapIn tx object", async () => {
      ...
      const result = await testee.swapIn(tokenIn, tokenOut, amountIn, amountOutMin);
    ...
    it("should throw if no exchange is found for the given tokens", async () => {
      ...
      await expect(testee.swapIn(tokenIn, tokenOut, oneInWei, oneInWei)).rejects.toThrow(
        `No exchange found for ${tokenIn} and ${tokenOut}`,
      );
    });
    ...
    it("should throw if a signer is not provided", async () => {
      const testee = await Mento.create(provider);
      await expect(
        testee.swapIn(fakeCeloUSDExchange.assets[0], fakeCeloUSDExchange.assets[1], oneInWei, oneInWei),
  ...
  describe("swapOut", () => {
    it("should return a populated swapOut tx object", async () => {
      ...
      const result = await testee.swapOut(tokenIn, tokenOut, amountOut, amountInMax);
      expect(result).toBe(fakePopulatedTxObj);
    ...
    it("should throw if no exchange is found for the given tokens", async () => {
      ...
      await expect(testee.swapOut(tokenIn, tokenOut, oneInWei, oneInWei)).rejects.toThrow(
        `No exchange found for ${tokenIn} and ${tokenOut}`,
      );
    });
    ...
    it("should throw if a signer is not provided", async () => {
      ...
      await expect(
        testee.swapOut(fakeCeloUSDExchange.assets[0], fakeCeloUSDExchange.assets[1], oneInWei, oneInWei),
      ).rejects.toThrow("A signer is required to populate the swapOut tx object");
    });
  });
});
