...
describe('Governance', () => {
  ...
  describe('createProposal', () => {
    ...
    it('should return a populated create proposal transaction', async () => {
        ...
        mockMentoGovernor.populateTransaction['propose(address[],uint256[],bytes[],string)'].mockReturnValue(fakeTxObj)
        const spy = jest
        ...
        expect(mockMentoGovernor.populateTransaction['propose(address[],uint256[],bytes[],string)']).toHaveBeenCalledTimes(1)
        expect(mockMentoGovernor.populateTransaction['propose(address[],uint256[],bytes[],string)']).toHaveBeenCalledWith(targets, values, calldatas, description)
        expect(spy).toHaveBeenCalledTimes(1)
        expect(spy).toHaveBeenCalledWith(fakeTxObj)

        expect(mockChainClient.populateTransaction).toHaveBeenCalledTimes(1)
        expect(mockChainClient.populateTransaction).toHaveBeenCalledWith(fakeTxObj)
    })
  ...
  describe('queueProposal', () => {
    it('should return a populated queue proposal transaction', async () => {
        ...
        mockMentoGovernor.populateTransaction['queue(uint256)'].mockReturnValue(fakeTxObj)
        const spy = jest
            .spyOn(mockChainClient, 'populateTransaction')
            // @ts-ignore
            .mockReturnValueOnce(fakePopulatedTxObj)
        ...
        expect(result).toEqual(fakePopulatedTxObj)
        expect(mockMentoGovernor.populateTransaction['queue(uint256)']).toHaveBeenCalledTimes(1)
        expect(mockMentoGovernor.populateTransaction['queue(uint256)']).toHaveBeenCalledWith(proposalId)
        expect(spy).toHaveBeenCalledTimes(1)
        expect(spy).toHaveBeenCalledWith(fakeTxObj)
  ...
  describe('executeProposal', () => {
    it('should return a populated execute proposal transaction', async () => {
        ...
        mockMentoGovernor.populateTransaction['execute(uint256)'].mockReturnValue(fakeTxObj)
        const spy = jest
            .spyOn(mockChainClient, 'populateTransaction')
            // @ts-ignore
            .mockReturnValueOnce(fakePopulatedTxObj)
        ...
        expect(result).toEqual(fakePopulatedTxObj)
        expect(mockMentoGovernor.populateTransaction['execute(uint256)']).toHaveBeenCalledTimes(1)
        expect(mockMentoGovernor.populateTransaction[ 'execute(uint256)']).toHaveBeenCalledWith(proposalId)
        expect(spy).toHaveBeenCalledTimes(1)
        expect(spy).toHaveBeenCalledWith(fakeTxObj)
  ...
  describe('castVote', () => {
    it('should return a populated cast vote transaction', async () => {
        ...
        expect(result).toEqual(fakePopulatedTxObj)
        expect(mockMentoGovernor.populateTransaction.castVote).toHaveBeenCalledTimes(1)
        expect(mockMentoGovernor.populateTransaction.castVote).toHaveBeenCalledWith(proposalId, support)
        expect(spy).toHaveBeenCalledTimes(1)
        expect(spy).toHaveBeenCalledWith(fakeTxObj)
  ...
  describe('cancelProposal', () => {
    it('should return a populated cancel proposal transaction', async () => {
        ...
        expect(result).toEqual(fakePopulatedTxObj)
        expect(mockMentoGovernor.populateTransaction.cancel).toHaveBeenCalledTimes(1)
        expect(mockMentoGovernor.populateTransaction.cancel).toHaveBeenCalledWith(proposalId)
        expect(spy).toHaveBeenCalledTimes(1)
        expect(spy).toHaveBeenCalledWith(fakeTxObj)
