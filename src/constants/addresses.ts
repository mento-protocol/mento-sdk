 ...
 */
export function getContractAddress(chainId: ChainId, contractName: keyof ContractAddresses): string {
	return addresses[chainId][contractName];
}

/**
 * Check if a contract is deployed on a given chain
 * @param chainId - The chain ID
 * @param contractName - The contract name
 * @returns True if the contract is deployed, false otherwise
 */
export function isContractDeployed(chainId: ChainId, contractName: keyof ContractAddresses): boolean {
	const address = addresses[chainId][contractName];
	return address !== "0x0" && address !== "0xNotDeployed";
}
