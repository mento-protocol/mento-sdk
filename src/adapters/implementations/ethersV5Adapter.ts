 ...
 *
 * Provides read and write operations using Ethers v5 provider and signer.
 */
export class EthersV5Adapter implements ProviderAdapter {
	...
	async estimateGas(options: ContractCallOptions): Promise<bigint> {
		try {
			...
			// Estimate gas
			const estimate = await contract.estimateGas[options.functionName](
				...(options.args || []),
			);
	...
	/**
	 * Normalize Ethers v5 TransactionResponse to our TransactionResponse interface
	 */
	...
	/**
	 * Normalize Ethers v5 TransactionReceipt to our TransactionReceipt interface
	 */
	private normalizeTransactionReceipt(
